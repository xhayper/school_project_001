...
export const createContext = async (event: H3Event) => {
  const supabaseConstant = {
    supabaseServiceRole: serverSupabaseServiceRole<Database>(event),
    supabaseSession: await serverSupabaseSession(event),
    supabaseClient: await serverSupabaseClient<Database>(event)
  };

  const userData: any = supabaseConstant.supabaseSession
    ? (
        await supabaseConstant.supabaseServiceRole
          .from('user_data')
          .select('*')
          .eq('id', supabaseConstant.supabaseSession.user.id)
          .single()
      ).data!
    : null;
